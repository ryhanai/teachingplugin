...
bool NodeConnectionInteraction::canConnect(PortIndex &portIndex, bool& typeConversionNeeded, std::unique_ptr<NodeDataModel> & converterModel) const {
...
bool NodeConnectionInteraction::tryConnect() const {
	DDEBUG("NodeConnectionInteraction::tryConnect");
...
nodePortIsEmpty(PortType portType, PortIndex portIndex) const
{
  ...
  // R.Hanai
  return ( portType == PortType::In && outPolicy == NodeDataModel::ConnectionPolicy::Many);
}
